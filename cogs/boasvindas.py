...
from utils.emojis import CHECK, X, USER, SETTINGS
import traceback
...
class Welcome(ui.LayoutView):
    def __init__(self, db: Database):
        ...
        container = discord.ui.Container(ui.TextDisplay(f'# {USER} Bem-vindo(a) ao Servidor!'))
        ...
        try:
            ...
            if not cargo_membro_id:
                await interaction.response.send_message(
                    f'{X} Cargo de membro não configurado. Use {SETTINGS} /config para definir.',
                    ephemeral=True
                )
                return
            ...
            if cargo is None:
                await interaction.response.send_message(f'{X} Cargo não encontrado', ephemeral=True)
                return

            if cargo in interaction.user.roles:
                await interaction.response.send_message(f'{X} Você já possui esse cargo', ephemeral=True)
                return

            await interaction.user.add_roles(cargo)
            await interaction.response.send_message(f'{CHECK} Bem vindo! Agora você pode acessar os canais.', ephemeral=True)
